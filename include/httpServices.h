...
String getTimezoneFromIP() {
  ...
  http.begin("http://ipwhois.app/json/");
  ...
  if (httpCode == HTTP_CODE_OK || HTTP_CODE_MOVED_PERMANENTLY) {
    ...
    if (err) {
      log_e("Parsing failed, err: ", err); // TODO: this doesn't actually print the error! log_e takes a format string, but err isn't a string anyway (err.f_str() is though) so a direct %s would crash
      return "";
    }

    String tz = JSONDocument["timezone"];
    return tz;
    
  } else {
    log_e("HTTP GET failled, code ", httpCode);
    return "";
  }
}
